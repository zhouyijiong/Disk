...
function appendTr(data,tag){
	...
	for(var i=0;i<data.size;i++){
		...
		switch(tag){
			case 'share':td2_input.setAttribute('disabled',false);
				break;
			case 'favorites':td2_input.setAttribute('disabled',false);
				break;
			default:
				if(file.shareLink){
...
function selectedOperting(obj){
	...
	switch(obj[obj.selectedIndex].value){
		case '==未选择==':break;
		case '下载文件':downloadFile(onlyId,fileSize);break;
		case '分享文件':shareFile(onlyId,fileSize);break;
		case '取消分享':cancelShare(onlyId,fileSize);break;
		case '分享链接':shareLink(fileName,onlyId,fileSize);break;
		case '查看链接':showLink(onlyId,fileSize);break;
		case '取消链接':cancelLink(onlyId,fileSize);break;
		case '删除文件':deleteFile(onlyId,fileSize);break;
		case '收藏文件':favoritesFile(onlyId,fileSize);break;
		case '取消收藏':cancelFavorite(onlyId,fileSize);break;
		default:alert('新手做的系统不容易,大佬破解手下留情');
	}
}
